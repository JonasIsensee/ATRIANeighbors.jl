...
function LibraryBenchmarkConfig(algorithm::Symbol, dataset_type::Symbol, N::Int, D::Int, k::Int;
    ...
    return LibraryBenchmarkConfig(algorithm, dataset_type, N, D, k, n_queries, trials, use_cache,
                                  min_points, leaf_size, hnsw_ef_construction, hnsw_ef_search, hnsw_M)
end
...
"""
    benchmark_query_time_lib(config::LibraryBenchmarkConfig, tree, queries::Matrix{Float64}) -> Tuple{Float64, Int}

Benchmark query time for all libraries.
Returns (median_query_time_seconds, estimated_distance_computations).
"""
function benchmark_query_time_lib(config::LibraryBenchmarkConfig, tree, queries::Matrix{Float64})
    ...
    if config.algorithm == :ATRIA
        ...
        avg_dist_calcs = config.N  # Placeholder - would need actual instrumentation
    ...
    elseif config.algorithm == :HNSW
        ...
        function run_hnsw_query()
            for i in 1:n_queries
                query = queries[i, :]
                HNSW.knn_search(tree, query, k)
            end
        end
        result = @benchmark $run_hnsw_query() samples=config.trials
        ...
        if config.algorithm == :BruteTree
            ...
            avg_dist_calcs = config.N ÷ 2  # Rough estimate for KDTree/BallTree
        end
    end
...
function run_library_benchmark(config::LibraryBenchmarkConfig; verbose::Bool=true)
    ...
    query_time, dist_calcs = benchmark_query_time_lib(config, tree, queries)
